...
test("table fullscreen keeps compact action controls available", async () => {
  const { readFile } = await import("node:fs/promises");
  const styles = await readFile(new URL("./styles.css", import.meta.url), "utf8");
  const appVue = await readFile(new URL("./App.vue", import.meta.url), "utf8");

  assert.match(appVue, /class="table-title-action-btn table-title-schema-btn"/);
  assert.match(appVue, /class="table-title-action-btn table-title-export-btn"/);
  assert.match(appVue, /fullscreen-schema-open/);
